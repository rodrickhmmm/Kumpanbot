...
class Gulag(commands.Cog):
    ...
    @app_commands.command(name="gulag", description="Odstraní roli uživateli a přidá mu gulag roli.")
    @app_commands.describe(
        uzivatel="Uživatel, kterého chceš poslat do gulagu"
    )
    async def gulag_slash(
        self, 
        ...
        if uzivatel.id == user.id:
            await interaction.response.send_message("Nemůžeš poslat do gulagu sám sebe!", ephemeral=True)
            return
        
        # Don't gulag the bot
        if uzivatel.bot:
            await interaction.response.send_message("Nemůžeš poslat bota do gulagu!", ephemeral=True)
            return
        
        # Hardcoded role IDs
        ROLE_TO_REMOVE_ID = 1368259286005841960
        ROLE_TO_ADD_ID = 1391857788019413032
        ...
        try:
            await uzivatel.remove_roles(role_to_remove, reason=f"Gulag od {user.name}")
            await uzivatel.add_roles(role_to_add, reason=f"Gulag od {user.name}")
            
            embed = discord.Embed(
                title="🏴 Do Gulagu!",
                description=f"**{uzivatel.mention}** byl/a poslán/a do gulagu!\n\n"
